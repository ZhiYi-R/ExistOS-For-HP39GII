...
tool were never imported) and emits a zero-overhead, type-safe C++23 model:
    ...
    generated/reg_verify.cpp  static_assert self-consistency gate over the model

The generated model lives entirely in ``namespace reg``. Originally reg_verify.cpp
included BOTH the new model and every legacy header in one TU (no union / macro
name clashes) and asserted bit-for-bit parity. That parity was proven green and
the legacy headers were then removed in Phase 5, so reg_verify.cpp now asserts the
model's own internal self-consistency instead (see emit_verify).

These ``generated/*.hpp`` are committed and ARE the source of truth; the live
build never runs this generator. It runs only when someone restores the legacy
regs*.h from git history to re-derive the model (then guarded in main()).
...
# simulation blocks, ...) were referenced by nothing and were dead code. Phase 5
# of the HAL migration removed ALL regs*.h — both those unmodelled ones and the
# modelled set below — leaving the committed generated/*.hpp as the source of
# truth; this generator parses regs*.h only when they are restored from git
# history. (Some unmodelled headers, e.g. regssimsspsel.h, even contained
# malformed duplicate-field unions that never compiled.)
INCLUDE = {
...
# Six (register, field) pairs whose legacy BP_/BM_/BV_ are mutually inconsistent
# in the vendor regs*.h and were transcribed verbatim. The original value-equality
# reg_verify proved the model matches the headers bit-for-bit and was green, so
# these are genuine silicon-header defects, not migration regressions or generator
# bugs. The self-consistency gate documents rather than asserts them: "fixing" the
# mask/pos would diverge the byte-exact image from the shipped firmware.
#   LCDIF_CTRL.SHIFT_NUM_BITS   BM 0x05000000 non-contiguous (bits 24,26) vs BP 25
#   LCDIF_CTRL.DATA_SWIZZLE     BM 0x00500000 non-contiguous (bits 20,22) vs BP 21
#   LCDIF_CTRL.DATA_SELECT      1-bit BM 0x00040000 but a 4-value enum (BV ..0x3)
#   LCDIF_CTRL.WORD_LENGTH      1-bit BM 0x00020000 but a 4-value enum (BV ..0x3)
#   LCDIF_CTRL.RUN              1-bit BM 0x00010000 but a 4-value enum (BV ..0x3)
#   POWER_CTRL.PSWITCH_IRQ_SRC  BM 0x08000000 (bit 27) disagrees with BP 21
KNOWN_FIELD_QUIRKS = {
    ("LCDIF_CTRL", "SHIFT_NUM_BITS"),
    ("LCDIF_CTRL", "DATA_SWIZZLE"),
    ("LCDIF_CTRL", "DATA_SELECT"),
    ("LCDIF_CTRL", "WORD_LENGTH"),
    ("LCDIF_CTRL", "RUN"),
    ("POWER_CTRL", "PSWITCH_IRQ_SRC"),
}


def emit_verify(regs):
    out = [BANNER,
    ...
    for r in regs:
        ...
        if r.union_text:
            ...
            out.append("static_assert(sizeof(reg::hw_%s_t) == 4u);" % tn)
            n += 1
        if r.multi:
            # Skip pure address-only aliases (e.g. a multi X_SET(n) bank carries no
            # COUNT, so reg::X_SET::count is 0); real registers carry fields/union.
            if r.fields or r.union_text:
                out.append("static_assert(reg::%s::address(1) > reg::%s::address(0));" %
                           (r.name, r.name))
                n += 1
                if r.set_addr:
            ...
            if r.set_addr:
                out.append("static_assert(reg::%s::has_atomic);" % r.name)
                out.append("static_assert(reg::%s_SET::address == reg::%s::address + 0x4u);" %
                           (r.name, r.name))
                n += 2
            if r.clr_addr:
                out.append("static_assert(reg::%s_CLR::address == reg::%s::address + 0x8u);" %
                           (r.name, r.name))
                n += 1
            if r.tog_addr:
                out.append("static_assert(reg::%s_TOG::address == reg::%s::address + 0xCu);" %
                           (r.name, r.name))
                n += 1
        for f in r.fields:
            if (r.name, f.name) in KNOWN_FIELD_QUIRKS:
                out.append("// quirk: %s.%s vendor-header BP_/BM_/BV_ inconsistent;" %
                           (r.name, f.name))
                out.append("//        preserved verbatim, not asserted "
                           "(see KNOWN_FIELD_QUIRKS).")
                nq += 1
                continue
            out.append("static_assert(field_ok<reg::%s_::%s::pos, reg::%s_::%s::mask>());" %
                       (r.name, f.name, r.name, f.name))
            n += 1
            for s, v in f.syms:
                out.append(
                    "static_assert(((uint32_t(reg::%s_sym::%s__%s) << reg::%s_::%s::pos)"
                    " & ~reg::%s_::%s::mask) == 0u);" %
                    (r.name, f.name, s, r.name, f.name, r.name, f.name))
                n += 1
    out.append("}  // namespace")
    out.append("")
    out.append("// total static_assert: %d" % n)
    out.append("// exempted vendor-header quirks: %d" % nq)
    out.append("")
    return "\n".join(out), n
...
def main():
    ...
    if not headers:
        sys.stderr.write(
            "gen_regmodel.py: no modelled regs*.h found in %s.\n"
            "They were removed in Phase 5 of the HAL migration; the committed\n"
            "generated/*.hpp are now the source of truth. To regenerate, restore\n"
            "the legacy headers from git history first, e.g.:\n"
            "  git checkout <pre-Phase5-rev> -- "
            "Bootloader/Hal/Hardware/registers/regs*.h\n"
            "Refusing to overwrite the committed model with an empty one.\n" % HERE)
        return
    regs = {}
    for h in headers:
    ...
    open(os.path.join(OUTDIR, "reg_values.hpp"), "w", newline="\n").write(
        emit_values(placed))
    verify_text, n_assert = emit_verify(placed)
